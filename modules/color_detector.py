...
#   • Motion-based fish tracker (real fish wins over reflection).
#   • Tracker filters by shape (rejects square wall-flicker blobs).
#   • Head direction from motion vector.
#   • Pass 1 (strict) / Pass 2 (regional) / Reject.
#   • IBC body ratio via interquartile depth.
...
# 30 = strong motion only (background wall flicker is usually <25).
MOTION_DIFF_THRESHOLD = 30

# Min blob area (px) in the motion mask to be a fish candidate
MIN_TRACK_BLOB_AREA = 150

# Max centroid jump between consecutive frames to link as same track
TRACK_MAX_JUMP_PX = 250

# Padding multiplier around tracked bbox
TRACK_BBOX_PAD = 1.6
...
def _per_frame_motion_blobs(frames_rgb: list[np.ndarray]) -> list[list[dict]]:
    ...
    Blob entry: {"centroid": (cx,cy), "bbox": (x0,y0,x1,y1), "area": int}
    """
    n = len(frames_rgb)
...
def _is_fish_shaped(b: dict) -> bool:
    """Reject square-ish or tiny blobs (wall flicker, JPEG noise)."""
    x0, y0, x1, y1 = b["bbox"]
    w = x1 - x0 + 1
    h = y1 - y0 + 1
    if w < 20 or h < 20:
...
def _track_largest_blob(blobs_per_frame: list[list[dict]]) -> list[Optional[dict]]:
    """
    Follow the largest fish-shaped motion blob across frames.
    """
