...
#   • Gemini ranks frames by (posture_class, flare_score, match_score)
#   • Top frames analyzed inside Gemini's bbox (reflection excluded)
#   • Grading: IBC rubric + Gemini deviations
...
# ============================================================
# POSTURE VALIDATION CONSTANTS (classical fallback)
# ============================================================
...
# Session 26H — AI path config
MAX_AI_ANALYZED_FRAMES = 10       # top N frames to run full analysis on
MIN_AI_USABLE_FRAMES = 1           # at least 1 usable → analyze


# ============================================================
# TRACKING CONSTANTS (classical fallback)
# ============================================================
...
# ============================================================
# MOTION TRACKING (classical fallback)
# ============================================================
...
# ============================================================
# FLARE DETECTION (classical fallback)
# ============================================================
...
# ============================================================
# POSTURE VALIDATION (classical fallback)
# ============================================================
...
# ============================================================
# CANDIDATE SPLIT + PICK (classical fallback)
# ============================================================
...
def _deviation_to_fault(deviation_text: str) -> str:
    ...
    slight | minor | major | severe | disqualifying | none
    """
    ...
    if any(w in t for w in ["major", "significant", "much ", "greatly"]):
        return "major"
    if any(w in t for w in ["narrow", "short", "long", "wide", "overlap", "spread",
                             "slope", "asymmetr", "mismatch", "slightly "]):
        # Anything with an actual measured deviation
        if "slight" in t or "minor" in t or "small" in t:
            return "slight"
        return "minor"
...
def compute_ibc_score(ai_best_verdict: dict,
                       consensus: dict) -> dict:
    """
    Combine Gemini deviations + classical measurements into an IBC score.
    Returns {"score": int, "grade": str, "deviations": [...],
             "faults_applied": [...]}.
    """
    try:
    ...
    # 1. Apply Gemini deviations as fault deductions
    ...
    for dev in deviations:
        ...
        score -= pts
        faults_applied.append({"source": "ai", "reason": dev,
                                "level": level, "points": pts})

    # 2. Cross-check classical measurements
    body_ratio = None
    try:
...
def analyze_photo(raw_bytes: bytes, k_clusters: int = 5,
    ...
    # Apply AI bbox first (Session 26H — reflection isolation)
    if real_fish_bbox:
        try:
    ...
    # Then motion hint (classical fallback)
...
def analyze_video(
    ...
    If AI available:
      • Gemini returns per-frame match_score, flare_score, posture_class, bbox
      • Frames ranked by (posture_class, flare_score, match_score)
      • Top MAX_AI_ANALYZED_FRAMES analyzed inside their bbox
      • Reject only if ALL frames are "unusable"
      • IBC grade computed from AI deviations + measurements
    ...
    if ai_verdicts:
        ...
        # Rank: posture_class desc, flare_score desc, match_score desc
        ...
        if len(usable) >= MIN_AI_USABLE_FRAMES:
            ...
            if analyses:
                # Split by head direction for side labelling
                ...
                # Compute IBC score using the highest-ranked AI verdict
